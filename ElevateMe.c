...
// This macro only affects the GCC and Clang linkers and aims to avoid unnecessary startup code in this app. If defined in this context, add these linker options:
// -Wl,-e,__start -nostartfiles
#define OVERRIDE_START

#if REGION(macro definitions)
#  if defined(OVERRIDE_START) && (defined(__GNUC__) || defined(__clang__)) && !defined(_MSC_VER)
#    define QUIT(status_) _exit((int)(status_))
#    if defined(__i386)
#      define MAIN __attribute__((noreturn)) void _start
#    else
#      define MAIN __attribute__((noreturn)) void __start
#    endif
MAIN(void);
#  else
...
#  define CLEANUP_WITH_HR_IF(cond_, hr_) \
    do                                   \
    {                                    \
      if (cond_)                         \
      {                                  \
...
static FORCE_INLINE BOOL IsSeparator(const WCHAR character)
{
  return character == L' ' || character == L'\t'; // space and tab are the default separators in a command line
}
#  define POINTS_TO_SEPARATOR(pchar_) IsSeparator(*pchar_)
#  define SKIP_SEPARATORS(pchar_) while (IsSeparator(*++pchar_))
...
static BSTR_BUF(identifier, ARRAYSIZE(TAGGED_UUID_PATTERN)) = {
  ...
  .bstr = { TAG_EVT } // tagged uuid string "T{XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX}", tag T is an alternating prefix: TAG_EVT (initially) or TAG_FMO
};
...
static FORCE_INLINE HRESULT RunScheduledTask(const VARIANT *const pTaskArg)
{
  ...
  hres = IRegisteredTask_Run(pRegTask, *pTaskArg, NULL); // pTaskArg ultimately references the TAG_ADM prefixed uuid string
...
static FORCE_INLINE HRESULT AsInvoker(const WCHAR *arg, const DWORD *const pLastError, const BYTE *const pProcParams)
{
  ...
  CLEANUP_WITH_HR_IF(!*arg, E_INVALIDARG); // no command received

  CLEANUP_IF_FAILED(MakeIdentifierUnique()); // append a uuid string to the TAG_EVT character in the identifier.bstr buffer
  ...
  RtlCopyMemory(strDest, arg, cmdLen * sizeof(WCHAR)); // command
  strDest += cmdLen;
  RtlCopyMemory(strDest, PROC_PARAM_VALUE_OF(curDirBuf), PROC_PARAM_VALUE_OF(curDirSize)); // current directory, the terminator is maintained in the zero-initialized view memory
  strDest += dirLen;
  RtlCopyMemory(strDest, PROC_PARAM_VALUE_OF(envBuf), PROC_PARAM_VALUE_OF(envSize)); // environment strings
...
{
  static PROCESS_INFORMATION procInf = { 0 };

  HRESULT hres = S_OK;
  CLEANUP_WITH_LAST_ERROR_IF(!(evt = OpenEventW(EVENT_MODIFY_STATE, FALSE, arg)));

  RtlCopyMemory(identifier.bstr, arg, identifier.nbytes); // this app is not meant to modify kernel-created data, hence the copy; the successfully opened event has proven that arg does indeed point to the identifier
  identifier.bstr[0] = TAG_FMO;
