...
| wait<br> (optional) | Integer value that specifies whether or not the elevated process is synchronized with the execution of the calling process.<br> Value 0 is for "false", any value >0 is for "true".<br> If omitted, the elevated process is not synchronized ("false"). That is, the calling process does not wait for the elevated process to terminate before it resumes the execution. |
